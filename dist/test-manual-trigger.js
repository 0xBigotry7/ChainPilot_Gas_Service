...
async function main() {
    try {
        // Initialize provider
        const provider = new ethers_1.ethers.JsonRpcProvider(process.env.ARBITRUM_RPC_URL);
        // Check wallet balance
        const balance = await provider.getBalance(walletAddress);
        logger.info(`💰 Current wallet balance: ${ethers_1.ethers.formatEther(balance)} ETH`);
        // Initialize funder wallet
        const funderWallet = new ethers_1.ethers.Wallet(process.env.FUNDER_PRIVATE_KEY, provider);
        logger.info(`🏦 Funding from wallet: ${funderWallet.address}`);
        // Get funder balance
        const funderBalance = await provider.getBalance(funderWallet.address);
        const funderBalanceEth = parseFloat(ethers_1.ethers.formatEther(funderBalance));
        logger.info(`💵 Funder wallet balance: ${generateProgressBar(funderBalanceEth)}`);
        // Calculate gas amount
        const gasAmount = ethers_1.ethers.parseEther(process.env.GAS_AMOUNT_TO_SEND || "0.001");
        const gasAmountEth = ethers_1.ethers.formatEther(gasAmount);
