...
const logger = (0, logger_1.createComponentLogger)('WalletMonitor');
// Helper function to generate a visual progress bar
function generateProgressBar(current, max = 0.05, length = 20) {
...
class WalletMonitor {
    constructor() {
        ...
        try {
            ...
            this.provider = new ethers_1.ethers.JsonRpcProvider(process.env.ARBITRUM_RPC_URL, undefined, {
                staticNetwork: true,
                polling: true,
            ...
            this.funderWallet = new ethers_1.ethers.Wallet(process.env.FUNDER_PRIVATE_KEY, this.provider);
            this.gasAmountToSend = ethers_1.ethers.parseEther(process.env.GAS_AMOUNT_TO_SEND || "0.001");
            ...
            this.whitelistedAddresses = new Set((process.env.WHITELISTED_ADDRESSES || '')
                .split(',')
    ...
    async checkFunderBalance() {
        try {
            ...
            const minBalanceEth = parseFloat(ethers_1.ethers.formatEther(minBalance));
    ...
    async setupDatabaseListener() {
        ...
        this.dbListener = supabase_1.supabase
    ...
    async start() {
        ...
        - Gas amount to send per airdrop: ${ethers_1.ethers.formatEther(this.gasAmountToSend)} ETH
    ...
    async sendGas(to) {
        try {
            ...
            const gasAmountEth = ethers_1.ethers.formatEther(this.gasAmountToSend);
            ...
            if (receipt && receipt.status === 1) {
                ...
                const remainingEth = parseFloat(ethers_1.ethers.formatEther(remainingBalance));
                const progressBar = generateProgressBar(remainingEth);
                logger.info(`✅ Gas airdrop confirmed! Remaining funder balance: ${progressBar}`);
                try {
                    await (0, supabase_1.updateWalletGasStatus)(to, tx.hash, gasAmountEth);
     ...
     */
    async processSingleWallet(walletAddress) {
        try {
