...
import { supabase, processUnfundedWallets } from './utils/supabase';
...
const captureLogsTransport = new winston.transports.Console({
...
winston.loggers.add('captureLogger', {
...
app.post('/process-unfunded', async (req, res) => {
  try {
    ...
    processUnfundedWallets(async (walletAddress) => {
